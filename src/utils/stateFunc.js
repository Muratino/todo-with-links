...
export const switchStatusArrTodos = (obj, criticalStatusArr, majorStatusArr, minorStatusArr) => {
  switch (obj.status) {
    case 'Critical':
...
export const fileterAndLocalCompletedTask = (state, action) => {
  const newArr = state.complatedTasks.filter(el => Number(el.id) !== Number(action.payload.id))
