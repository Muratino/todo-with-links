...
export const LinkBlock = () => {
  const { links, typeParent, filterLinks, isFilteringLinks } = useSelector(state => state.links)
  const dispatch = useDispatch();

  const createNewParent = () => {
    const parent = {
      parentSummary: typeParent.parentSummary ? typeParent.parentSummary : 'Parent title',
    ...
    <CardContent>
      <Box sx={{
        ...
        <IconButton
          ...
          sx={{ padding: 0, width: '20px', position: 'absolute', bottom: '6px', right: '20px' }}>
