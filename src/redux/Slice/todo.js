...
import {
  createStatusArrTodos,
  fileterAndLocalCompletedTask,
  fileterAndLocalTodos,
  findObjTodo,
  switchStatusArrTodos,
} from "../../utils/stateFunc";
...
export const todoSlice = createSlice({
  ...
  reducers: {
    ...
    setChangeTaskStatus(state, action) {
      ...
      const { criticalStatusArr, majorStatusArr, minorStatusArr } =
        createStatusArrTodos(state, action);

      changedStatus.status = action.payload.status;

      state.todos = switchStatusArrTodos(
        changedStatus,
        criticalStatusArr,
        majorStatusArr,
        minorStatusArr
    ...
    setComplatedTask(state, action) {
      const completTask = state.todos.filter(
        (el) => Number(el.id) === Number(action.payload)
      );
      const { newArr } = fileterAndLocalTodos(state, action);

      const criticalStatusArr = state.complatedTasks.filter(
        (el) =>
          String(el.status) === "Critical" &&
          Number(el.id) !== Number(action.payload)
      );
      const majorStatusArr = state.complatedTasks.filter(
        (el) =>
          String(el.status) === "Major" &&
          Number(el.id) !== Number(action.payload)
      );
      const minorStatusArr = state.complatedTasks.filter(
        (el) =>
    ...
    setReturnTask(state, action) {
      const task = state.complatedTasks.filter(
